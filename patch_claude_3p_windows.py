...
FRONTEND_ASSETS_REL = Path("resources/app.asar.unpacked/assets/v1")  # May differ
...
def main():
    ...
    print(f"ASAR: {asar_report['status']}")
    for p in asar_report["patches"]:
        status = p["status"]
        label = p["label"]
        file = p.get("file", "")
        print(f"  {status:15} {label} [{file}]")
    ...
    print(f"\n{'=' * 72}")
    print("Windows 3P mode notes:")
    print("  - Code signing: Not required for local Electron apps")
    print("  - Registry policy: HKLM\\SOFTWARE\\Policies\\Anthropic\\Claude")
    print("  - Localization: Use --zh-cn (TODO: port from macOS script)")
    print(f"{'=' * 72}")
